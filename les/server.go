...
import (
	...
	"github.com/epvchain/go-epvchain/les/flowcontrol"
...
func NewLesServer(epv *epv.EPVchain, config *epv.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(epv.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, epv.EventMux(), epv.Engine(), newPeerSet(), epv.BlockChain(), epv.TxPool(), epv.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(epv.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(epv.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(epv.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(epv.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(epv.ChainDb())
	return srv, nil
}
...
func newCostStats(db epvdb.Database) *requestCostStats {
