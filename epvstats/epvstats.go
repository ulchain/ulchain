...
// Package epvstats implements the network stats reporting service.
package epvstats
...
import (
	...
	"github.com/epvchain/go-epvchain/common/mclock"
...
// Service implements an EPVchain netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	epv    *epv.EPVchain      // Full EPVchain service if monitoring a full node
	les    *les.LightEPVchain // Light EPVchain service if monitoring a light node
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // EPVstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["epv"]; info != nil {
		network = fmt.Sprintf("%d", info.(*epv.NodeInfo).Network)
		protocol = fmt.Sprintf("epv/%d", epv.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the epvstats server
	start := time.Now()
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.epv != nil {
		...
		header = block.Header()
		td = s.epv.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.epv != nil {
		mining = s.epv.Miner().Mining()
		hashrate = int(s.epv.Miner().HashRate())

		sync := s.epv.Downloader().Progress()
		syncing = s.epv.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.epv.ApiBackend.SuggestPrice(context.Background())
