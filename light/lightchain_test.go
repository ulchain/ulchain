...
func makeHeaderChain(parent *types.Header, n int, db epvdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), epvhash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (epvdb.Database, *LightChain, error) {
	db, _ := epvdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db}, gspec.Config, epvhash.NewFaker())
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, epvhash.NewFullFaker())
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() epvdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, epvhash.NewFaker())
	if err != nil {
