...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the EPVchain
...
type wallet struct {
	...
	deriveChain    epvchain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived EPVchain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain epvchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
