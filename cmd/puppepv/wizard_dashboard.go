...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"epvstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "epvstats":
				if infos, err := checkEPVstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have epvstats running, ask whether to make the secret public or not
	if w.conf.epvstats != "" {
		fmt.Println()
		fmt.Println("Include epvstats secret on dashboard (y/n)? (default = yes)")
