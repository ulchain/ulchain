...
FROM puppepv/explorer:latest

ADD epvstats.json /epvstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../epv-net-intelligence-api && pm2 start /epvstats.json)' >  explorer.sh && \
	echo '(cd ../epvcchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEPVstats is the configuration file for the epvstats javascript client.
var explorerEPVstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.epvchain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.EPVstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	epvstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEPVstats)).Execute(epvstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.epvstats[:strings.Index(config.epvstats, ":")],
		"Secret": config.epvstats[strings.Index(config.epvstats, ":")+1 : strings.Index(config.epvstats, "@")],
		"Host":   config.epvstats[strings.Index(config.epvstats, "@")+1:],
	})
	files[filepath.Join(workdir, "epvstats.json")] = epvstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.epvchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		epvstats: infos.envvars["STATS"],
	}
	return stats, nil
}
