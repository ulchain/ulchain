...
FROM puppepv/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gepv --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gepv --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --epvstats \'{{.EPVstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppepvNetworkID/{{.NetworkID}}/g' dist/js/epvcwallet-master.js && \
	sed -i 's/PuppepvNetwork/{{.Network}}/g'     dist/js/epvcwallet-master.js && \
	sed -i 's/PuppepvDenom/{{.Denom}}/g'         dist/js/epvcwallet-master.js && \
	sed -i 's/PuppepvHost/{{.Host}}/g'           dist/js/epvcwallet-master.js && \
	sed -i 's/PuppepvRPCPort/{{.RPCPort}}/g'     dist/js/epvcwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:7545"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.epvchain
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["7545/tcp"]
