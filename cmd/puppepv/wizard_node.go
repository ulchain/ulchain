...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.epvstats == "" {
		log.Error("No epvstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			...
			infos = &nodeInfos{port: 50303, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.EPVhash != nil && !boot {
		fmt.Println()
		if infos.epvhashdir == "" {
			fmt.Printf("Where should the epvhash mining DAGs be stored on the remote machine?\n")
			infos.epvhashdir = w.readString()
		} else {
			fmt.Printf("Where should the epvhash mining DAGs be stored on the remote machine? (default = %s)\n", infos.epvhashdir)
			infos.epvhashdir = w.readDefaultString(infos.epvhashdir)
		}
	}
	...
	if infos.epvstats == "" {
		...
		infos.epvstats = w.readString() + ":" + w.conf.epvstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.epvstats)
		infos.epvstats = w.readDefaultString(infos.epvstats) + ":" + w.conf.epvstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.EPVhash != nil {
			// EPVhash based miners only need an epvcbase to mine against
			fmt.Println()
			if infos.epvcbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.epvcbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.epvcbase)
				infos.epvcbase = w.readDefaultAddress(common.HexToAddress(infos.epvcbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy EPVchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
