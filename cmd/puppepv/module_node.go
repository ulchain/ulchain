...
// nodeDockerfile is the Dockerfile required to run an EPVchain node.
var nodeDockerfile = `
FROM epvchain/client-go:latest
...
{{end}}
RUN \
  echo 'gepv --cache 512 init /genesis.json' > gepv.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.epvchain/keystore/ && cp /signer.json /root/.epvchain/keystore/' >> gepv.sh && \{{end}}
	echo $'gepv --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --epvstats \'{{.EPVstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .EPVCbase}}--epvcbase {{.EPVCbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gepv.sh

ENTRYPOINT ["/bin/sh", "gepv.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an EPVchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.epvchain{{if .EPVhashdir}}
      - {{.EPVhashdir}}:/root/.epvhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new EPVchain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.epvcbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"EPVhashdir":  config.epvhashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"EPVstats":   config.epvstats[:strings.Index(config.epvstats, ":")],
		"EPVCbase":  config.epvcbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"EPVstats username":        info.epvstats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gepv --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.epvchain"],
		epvhashdir:  infos.volumes["/root/.epvhash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		epvstats:   infos.envvars["STATS_NAME"],
		epvcbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
