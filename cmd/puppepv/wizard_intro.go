...
// makeWizard creates and returns a new puppepv wizard.
...
// setting up a new or managing an existing EPVchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppepv, your EPVchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new EPVchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and epvstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppepv uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering EPVchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppepv", w.network)
