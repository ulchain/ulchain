...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for epvstats availability")
	if infos, err := checkEPVstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["epvstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["epvstats"] = infos.Report()
		epvstats = infos.config
	}
