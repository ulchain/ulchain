...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many EPVCs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.epvstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.epvstats = w.readString() + ":" + w.conf.epvstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.epvstats)
		infos.node.epvstats = w.readDefaultString(infos.node.epvstats) + ":" + w.conf.epvstats
	}
