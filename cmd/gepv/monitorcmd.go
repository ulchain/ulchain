...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The Gepv monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	// Attach to an EPVchain node over IPC or RPC
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gepv (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached gepv node and retrieves the entire set
