...
func TestAccountListEmpty(t *testing.T) {
	gepv := runGepv(t, "account", "list")
	gepv.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gepv := runGepv(t, "account", "list", "--datadir", datadir)
	defer gepv.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gepv := runGepv(t, "account", "new", "--lightkdf")
	defer gepv.ExpectExit()
	gepv.Expect(`
...
`)
	gepv.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gepv := runGepv(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gepv.ExpectExit()
	gepv.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gepv.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gepv.StderrText(), m) {
...
// https://github.com/epvchain/go-epvchain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gepv.SetTemplateFunc("keypath", func(file string) string {
