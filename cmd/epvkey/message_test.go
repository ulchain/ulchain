...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "epvkey-test")
	if err != nil {
	...
	// Create the key.
	generate := runEPVkey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runEPVkey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runEPVkey(t, "verifymessage", address, signature, message)
