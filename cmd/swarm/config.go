...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/epvchain/go-epvchain/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (gepv --datadir)
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(GEPV_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	// exit if the deprecated --epvapi flag is set
	if ctx.GlobalString(DeprecatedEPVAPIFlag.Name) != "" {
		utils.Fatalf("--epvapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
}
