...
import (
	...
	"github.com/epvchain/go-epvchain/console"
	"github.com/epvchain/go-epvchain/contracts/ens"
	"github.com/epvchain/go-epvchain/crypto"
	"github.com/epvchain/go-epvchain/epvclient"
	"github.com/epvchain/go-epvchain/internal/debug"
	...
	"github.com/epvchain/go-epvchain/swarm"
	bzzapi "github.com/epvchain/go-epvchain/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the EPVchain API provider to use to settle SWAP payments",
	...
	EnsAPIFlag = cli.StringFlag{
		Name:   "ens-api",
		Usage:  "URL of the EPVchain API provider to use for ENS record lookups",
	...
	DeprecatedEPVAPIFlag = cli.StringFlag{
		Name:  "epvapi",
...
// This init function sets defaults so cmd/swarm can run alongside gepv.
func init() {
...
var app = utils.NewApp(gitCommit, "EPVchain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-epvchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.epvchain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.epvchain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.epvchain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.epvchain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gepv only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gepv and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the epvchain node
	...
	//register BZZ as node.Service in the epvchain node
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := epvclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if bzzconfig.EnsApi != "" {
			...
			ensClient = epvclient.NewClient(client)
	...
	//register within the epvchain node
