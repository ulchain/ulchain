...
// faucet is a EPVC faucet backed by a light client.
package main
...
var (
	...
	epvPortFlag = flag.Int("epvport", 50303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the EPVchain protocol")
	statsFlag   = flag.String("epvstats", "", "EPVstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of EPVCs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for EPVchain and the faucet")
)

var (
	epvc = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s EPVCs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *epvPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // EPVchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an EPVchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // EPVchain protocol stack
	client *epvclient.Client   // Client connection to the EPVchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the EPVchain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := epv.DefaultConfig
	...
	// Assemble the epvstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEPVchain
			ctx.Service(&serv)
			return epvstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := epvclient.NewClient(api)
...
// close terminates the EPVchain connection and tears down the faucet.
...
// apiHandler handles requests for EPVC grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, epvc),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the EPVchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/epvchain/go-epvchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), epvc)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, epvc)
...
// the username, avatar URL and EPVchain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for EPVchain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No EPVchain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the EPVchain address and profile URL.
...
// returning the username, avatar URL and EPVchain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain EPVchain address,
