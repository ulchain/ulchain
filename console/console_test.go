...
func newTester(t *testing.T, confOverride func(*epv.Config)) *tester {
	...
	// Create a networkless protocol stack and start an EPVchain service within
	...
	epvConf := &epv.Config{
		...
		EPVCbase: common.HexToAddress(testAddress),
		EPVhash: epvhash.Config{
			PowMode: epvhash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(epvConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return epv.New(ctx, epvConf) }); err != nil {
		t.Fatalf("failed to register EPVchain protocol: %v", err)
	}
