...
import (
	...
	"github.com/epvchain/go-epvchain/internal/jsre"
	"github.com/epvchain/go-epvchain/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jepv", struct{}{})

	jepvObj, _ := c.jsre.Get("jepv")
	jepvObj.Object().Set("send", bridge.Send)
	jepvObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jepv);"); err != nil {
	...
	flatten := "var epv = web3.epv; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = epv.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jepv.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jepv.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jepv.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jepv.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jepv.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines epv.getBalance(epv.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gepv instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gepv metadata
	fmt.Fprintf(c.printer, "Welcome to the Gepv JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + epv.coinbase);
		console.log("at block: " + epv.blockNumber + " (" + new Date(1000 * epv.getBlock(epv.blockNumber).timestamp) + ")");
