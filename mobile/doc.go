...
// Package gepv contains the simplified mobile APIs to go-epvchain.
//
// The scope of this package is *not* to allow writing a custom EPVchain client
// with pieces plucked from go-epvchain, rather to allow writing native dapps on
...
// from the go-epvchain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-epvchain version.
