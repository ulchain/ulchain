...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gepv.LightScryptN, Gepv.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.epvchain", new NodeConfig());
			node.start();
			...
			EPVchainClient ec = node.getEPVchainClient();
	...
	// signatures too. Regression test for go-epvchain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gepv-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/epvchain/gepvtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gepv and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.epvchain", "github.com/epvchain/go-epvchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gepv.aar"), "gepv.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "epvchain", "gepvtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
