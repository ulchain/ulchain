...
func newTestBlockChain(fake bool) *BlockChain {
	...
	gspec.MustCommit(db)
	engine := epvhash.NewFullFaker()
	if !fake {
		engine = epvhash.NewTester()
	}
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, blockchain2, err := newCanonical(epvhash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, epvhash.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, epvhash.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	...
	block := makeBlockChain(bchain.CurrentBlock(), 1, epvhash.NewFaker(), bchain.db, 0)[0]
	bchain.insert(block)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, processor, err := newCanonical(epvhash.NewFaker(), length, full)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, blockchain, err := newCanonical(epvhash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, epvhash.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, epvhash.NewFaker(), db, forkSeed)[1:]
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(bc.db, nil, bc.chainConfig, epvhash.NewFaker(), vm.Config{})
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		db, blockchain, err := newCanonical(epvhash.NewFaker(), 0, full)
		if err != nil {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, epvhash.NewFaker(), db, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = epvhash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, epvhash.NewFaker(), db, 0)
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
	...
	archiveDb, _ := epvdb.NewMemDatabase()
	gspec.MustCommit(archiveDb)
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, epvhash.NewFaker(), vm.Config{})
	defer archive.Stop()
	...
	fastDb, _ := epvdb.NewMemDatabase()
	gspec.MustCommit(fastDb)
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, epvhash.NewFaker(), vm.Config{})
	defer fast.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), gendb, int(height), nil)
	...
	lightDb, _ := epvdb.NewMemDatabase()
	gspec.MustCommit(lightDb)

	light, _ := NewBlockChain(lightDb, nil, gspec.Config, epvhash.NewFaker(), vm.Config{})
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, epvhash.NewFaker(), vm.Config{})
	...
	chain, _ = GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, epvhash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, epvhash.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(chain); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), db, 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(bc.chainConfig, bc.genesisBlock, epvhash.NewFaker(), bc.db, 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], epvhash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, epvhash.NewFaker(), db, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.NewEIP155Signer(gspec.Config.ChainId)
...
//
// https://github.com/epvchain/go-epvchain/pull/15941
