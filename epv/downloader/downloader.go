...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [epv/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [epv/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [epv/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [epv/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [epv/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [epv/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [epv/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [epv/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb epvdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() epvchain.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "epv", p.version, "head", hash, "td", td, "mode", d.mode)
