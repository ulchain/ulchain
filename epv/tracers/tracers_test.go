...
import (
	...
	"github.com/epvchain/go-epvchain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gepv console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = epv.getBlock(epv.getTransaction(tx).blockHash);
  var genesis = epv.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.epv.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  epv.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
