...
func testGetNodeData(t *testing.T, protocol int) {
	...
	statedb, _ := epvdb.NewMemDatabase()
...
// Tests that post epv protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", epv63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, epvhash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
