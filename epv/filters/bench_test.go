...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/gepv/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := epvdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = epvdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db epvdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
...
func clearBloomBits(db epvdb.Database) {
