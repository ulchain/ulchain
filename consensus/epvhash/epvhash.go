...
// Package epvhash implements the epvhash proof-of-work consensus engine.
package epvhash
...
var (
	...
	// sharedEPVhash is a full instance that can be shared between multiple users.
...
// cache wraps an epvhash cache with some metadata to allow easier concurrent use.
type cache struct {
...
// newCache creates a new epvhash verification cache and returns it as a plain Go
...
// dataset wraps an epvhash dataset with some metadata to allow easier concurrent use.
type dataset struct {
	epoch   uint64    // Epoch for which this cache is relevant
	dump    *os.File  // File descriptor of the memory mapped cache
	mmap    mmap.MMap // Memory map itself to unmap before releasing
...
// newDataset creates a new epvhash mining dataset and returns it as a plain Go
// interface to be usable in an LRU cache.
...
// MakeCache generates a new epvhash cache and optionally stores it to disk.
func MakeCache(block uint64, dir string) {
	c := cache{epoch: block / epochLength}
	c.generate(dir, math.MaxInt32, false)
}

// MakeDataset generates a new epvhash dataset and optionally stores it to disk.
func MakeDataset(block uint64, dir string) {
	d := dataset{epoch: block / epochLength}
	d.generate(dir, math.MaxInt32, false)
}

// Mode defines the type and amount of PoW verification an epvhash engine makes.
type Mode uint
...
// Config are the configuration parameters of the epvhash.
type Config struct {
	CacheDir       string
	CachesInMem    int
	CachesOnDisk   int
...
// EPVhash is a consensus engine based on proot-of-work implementing the epvhash
// algorithm.
type EPVhash struct {
	config Config
	...
	shared    *EPVhash       // Shared PoW verifier to avoid cache regeneration
...
// New creates a full sized epvhash PoW scheme.
...
// NewTester creates a small sized epvhash PoW scheme useful only for testing
// purposes.
func NewTester() *EPVhash {
	return New(Config{CachesInMem: 1, PowMode: ModeTest})
}

// NewFaker creates a epvhash consensus engine with a fake PoW scheme that accepts
// all blocks' seal as valid, though they still have to conform to the EPVchain
// consensus rules.
...
// NewFakeFailer creates a epvhash consensus engine with a fake PoW scheme that
// accepts all blocks as valid apart from the single one specified, though they
// still have to conform to the EPVchain consensus rules.
...
// NewFakeDelayer creates a epvhash consensus engine with a fake PoW scheme that
// accepts all blocks as valid, but delays verifications by some time, though
// they still have to conform to the EPVchain consensus rules.
...
// NewFullFaker creates an epvhash consensus engine with a full fake scheme that
...
// NewShared creates a full sized epvhash PoW shared between all requesters running
// in the same process.
