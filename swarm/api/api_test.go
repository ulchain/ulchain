...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.epv"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.epv"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.epv"`),
		},
	}
	for _, x := range tests {
