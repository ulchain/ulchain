...
#
# The cluster contains a bootnode, a gepv node and multiple swarm nodes, with
...
# static gepv configuration
GEPV_IP="192.168.33.3"
GEPV_RPC_PORT="7545"
GEPV_RPC_URL="http://${GEPV_IP}:${GEPV_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gepv and
# swarm nodes
start_bootnode() {
...
# start_gepv_node starts a gepv node with --datadir pointing at <base-dir>/gepv
# and a single, unlocked account with password "gepv"
start_gepv_node() {
  local dir="${base_dir}/gepv"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gepv account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GEPV_IP}"
    --rpcport   "${GEPV_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gepv" "${GEPV_IP}" "$(which gepv)" ${args[@]}
}
...
create_account() {
  ...
  gepv --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
