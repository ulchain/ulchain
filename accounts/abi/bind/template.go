...
import "github.com/epvchain/go-epvchain/accounts/abi"
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new EPVchain contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an EPVchain contract.
	type {{.Type}} struct {
	...
	// {{.Type}}Caller is an auto generated read-only Go binding around an EPVchain contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an EPVchain contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Filterer is an auto generated log filtering Go binding around an EPVchain contract events.
	type {{.Type}}Filterer struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an EPVchain contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an EPVchain contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an EPVchain contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an EPVchain contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an EPVchain contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an EPVchain contract.
	...
	{{range .Events}}
		...
		type {{$contract.Type}}{{.Normalized.Name}}Iterator struct {
			...
			sub  epvchain.Subscription // Subscription for errors, completion and termination
...
import org.epvchain.gepv.*;
import org.epvchain.gepv.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new EPVchain contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, EPVchainClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gepv.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, Gepv.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(Gepv.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// EPVchain address where this contract is located at.
		public final Address Address;

		// EPVchain transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, EPVchainClient client) throws Exception {
			this(Gepv.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gepv.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, Gepv.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = Gepv.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Gepv.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = Gepv.newCallOpts();
				}
