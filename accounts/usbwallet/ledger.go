...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and EPVchain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an EPVchain transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "EPVchain app in browser mode", w.failure
	}
	if w.offline() {
		return "EPVchain app offline", w.failure
	}
	return fmt.Sprintf("EPVchain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the EPVchain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// EPVchain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the EPVchain app's version, will fail prior to v1.0.2
...
//
// Note, if the version of the EPVchain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the EPVchain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the EPVchain wallet app running
...
// ledgerDerive retrieves the currently active EPVchain address from a Ledger
...
//   EPVchain address length | 1 byte
//   EPVchain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the EPVchain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an EPVchain address and return
