...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout epv_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for EPVSubscribe requests
}
...
// EPVSubscribe registers a subscripion under the "epv" namespace.
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. EPVSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through EPVSubscribe.
