	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("EPVSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("EPVSubscribe shouldn't have panicked for %#v", arg)
		...
		client.EPVSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("epv", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.EPVSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while EPVSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("epv", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.EPVSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("EPVSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("EPVSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("EPVSubscribe did not return within 1s after Close")
	}
}
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.EPVSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "epv_echo", i)
			if err != nil {
				if !wantError {
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-epvchain-test-ipc-%d-%d", os.Getpid(), rand.Int63())
