...
func (s *State) addToKillRing(text []rune, mode int) {
	...
	if mode == 0 { 
		...
		} else if s.killRing.Len() >= KillRingMax { 
		...
		case rune:
			...
			case ctrlK: 
				if pos >= len(line) {
					...
					if killAction > 0 {
						s.addToKillRing(line[pos:], 1) 
					} else {
						s.addToKillRing(line[pos:], 0) 
					}
			...
			case ctrlW: 
				...
				if killAction > 0 {
					s.addToKillRing(newBuf, 2) 
				} else {
					s.addToKillRing(newBuf, 0) 
				}
				killAction = 2 
