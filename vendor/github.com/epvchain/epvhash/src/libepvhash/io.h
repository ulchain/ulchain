...
// Reference: https://github.com/epvchain/wiki/wiki/EPVhash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see epvhash_io_prepare
enum epvhash_io_rc {
	EPVHASH_IO_FAIL = 0,           ///< There has been an IO failure
	EPVHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	EPVHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	EPVHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of epvhash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * epvhash_full_t
 */
#ifdef EPVHASH_PRINT_CRITICAL_OUTPUT
#define EPVHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("EPVHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for epvhash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the epvhash
 ...
 *                           https://github.com/epvchain/wiki/wiki/EPVhash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum epvhash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/epvchain/wiki/wiki/EPVhash-DAG
 *
...
static inline bool epvhash_io_mutable_name(
	uint32_t revision,
